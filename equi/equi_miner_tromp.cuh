...
//
// Adapted for ccminer WS3: this is an include-fragment pulled into
// cuda_equi_tromp.cu inside `namespace tromp144`. Its own top-level includes
// (equi.h, blake2b.cu) are provided by the wrapper TU before this file, so they
// are removed here. main() is stripped (replaced by the wrapper's solve entry).
...
struct equi {
  ...
  void setheadernonce(const char *headernonce, const u32 len, const char *personal) {
...
#if 0  // tromp standalone main() disabled; cuda_equi_tromp.cu provides the solve entry
